matchpos <- function(tomatch, ref.df,
  ...
  #' Function to match a set of variants to a reference
  ...
  match.col <- function(test, target, default.targets, auto.detect=T, silent=F) {
    mustfind <- target != ""
    if(mustfind) {
      if(is.numeric(target)) return(target)
      test2 <- test
      targets2 <- target
    } else if(auto.detect) {
  ...
  alt.col <- match.col(colnames.tomatch, alt, c("alt", "a2"), auto.detect.tomatch, silent=silent)
  ref.col <- match.col(colnames.tomatch, ref, c("ref", "a1"), auto.detect.tomatch, silent=silent)
  ...
  ref.alt.col <- match.col(colnames.ref, ref.alt, c("alt", "a2"), auto.detect.ref, silent=silent)
  ref.ref.col <- match.col(colnames.ref, ref.ref, c("ref", "a1"), auto.detect.ref, silent=silent)
